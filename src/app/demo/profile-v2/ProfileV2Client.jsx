...
/**
 * ProfileV2Client — premium creator profile redesign (visual demo).
 *
 * Layout map:
 *   ┌──────────────────────────────────────────────────────────┐
 *   │  /demo bar (yellow strip + reset state)                  │
 *   ├──────────────────────────────────────────────────────────┤
 ...
 *   └──────────────────────────────────┴───────────────────────┘
 *
 * Edit Profile modal: right-side drawer on desktop, full-screen
 * on mobile. Sections expandable: Identity / Profile / Media /
 * Social / Privacy / Notifications / Account. All inputs wired
 * to local state; save shows a "Saved to demo state" toast.
 */
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: TEXT, fontFamily: "Inter,sans-serif" }}>
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${LIME_RING}`,
        padding: "8px 16px",
        fontSize: 11,
        fontWeight: 700,
        ...
        <span>/demo/profile-v2 · staging preview · Phase 1 (visual)</span>
        <Link
          href="/profile"
          style={{ color: SUB, fontSize: 11, textDecoration: "none", fontWeight: 600 }}
        >
          ⟵ compare with live /profile
        </Link>
      </div>
      ...
      <main style={{ maxWidth: 1180, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        {/* Demo notes — visible at the bottom so reviewer knows
            what's wired vs deferred to Phase 2. */}
        <section style={{
          marginTop: 28,
          padding: "16px 18px",
          background: CARD,
          border: `1px dashed ${LIME_RING}`,
          borderRadius: 12,
          fontSize: 12.5,
          color: SUB,
          lineHeight: 1.55,
        }}>
          <div style={{ color: LIME, fontWeight: 800, marginBottom: 6 }}>What&rsquo;s wired in this demo</div>
          <ul style={{ margin: "4px 0 0 18px", padding: 0 }}>
            <li>Hero — cover gradient + your real avatar + name + member-since + credits.</li>
            <li>Edit profile modal — every field from the spec, drag-to-reorder social links, saves to local state only.</li>
            <li>Right sidebar — plan / credits / referral / account shortcuts.</li>
          </ul>
          <div style={{ color: LIME, fontWeight: 800, margin: "12px 0 6px" }}>Phase 2 (after approval)</div>
          <ul style={{ margin: "4px 0 0 18px", padding: 0 }}>
            <li>Schema: add <code style={codePill}>bio / location / tagline / socialLinks / coverImageUrl / pronouns</code> to seedance&rsquo;s User model — community + music + edits already share this row, so changes propagate the moment they&rsquo;re committed.</li>
            <li><code style={codePill}>PATCH /api/me</code> for the edit save, with server-side username uniqueness + 30-day cooldown.</li>
            <li>Cover banner upload to R2 (avatar can migrate too — currently 250×250 base64 in DB).</li>
            <li>Optional Pusher push so other open tabs (community / music / edits) update live without refresh.</li>
          </ul>
        </section>
      </main>
...
// EDIT PROFILE DRAWER
// ════════════════════════════════════════════════════════════════
function EditProfileDrawer({ draft, setDraft, onClose, onSave }) {
  const titleId = useId();
        ...
        <div style={{
          ...
          <Section title="Media">
            <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <button type="button" style={smallBtn} onClick={() => alert("Avatar upload — wired in Phase 2 (R2 + crop)")}>
                  Change avatar
                </button>
                <p style={fineHint}>JPG, PNG or WebP. Face-aware crop suggested.</p>
              </div>
            </div>
            <div style={{ marginTop: 14 }}>
              <button type="button" style={smallBtn} onClick={() => alert("Cover upload — wired in Phase 2 (R2 + 16:9 crop)")}>
                Change cover banner
              </button>
              <p style={fineHint}>16:9 cinematic. Until Phase 2 we use a brand gradient placeholder.</p>
            </div>
          </Section>
