...
export async function GET(req) {
  ...
  // Allowlist: only proxy URLs that point at our own R2 bucket. Stops
  // anyone using this route as an open redirect / bandwidth amplifier.
  const r2Public = process.env.R2_PUBLIC_URL?.replace(/\/$/, "");
  if (!r2Public || !target.startsWith(r2Public + "/")) {
