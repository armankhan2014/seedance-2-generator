"use client";
/**
 * SignInModal — global sign-in modal triggered via CustomEvent "openSignIn".
 * Usage from anywhere: window.dispatchEvent(new CustomEvent("openSignIn"))
 * Added to layout.js so it's always mounted.
 */
...
const PROVIDERS = [
  ...
  },
  {
    ...
    style: { background: "#24292e", color: "#fff", border: "1px solid rgba(255,255,255,0.15)" },
    hoverStyle: { background: "#2f363d" },
  },
];
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: "14px",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: "24px", boxShadow: "0 8px 24px rgba(124,58,237,0.4)",
          }}>🎬</div>

          <h2 style={{ margin: "0 0 6px", fontSize: "1.2rem", fontWeight: 800, color: "#fff", letterSpacing: "-0.3px" }}>
            Welcome to Seedance
          </h2>
          <p style={{ margin: 0, fontSize: "0.82rem", color: "rgba(196,181,253,0.8)", lineHeight: 1.5 }}>
            Sign in to start generating AI videos
          </p>
        </div>

        {/* Provider buttons */}
        <div style={{ padding: "24px 24px 20px", display: "flex", flexDirection: "column", gap: "10px" }}>
          {PROVIDERS.map(p => (
            <button
              key={p.id}
              ...
              style={{
                ...
                ...(hovered === p.id ? { ...p.style, ...p.hoverStyle } : p.style),
              }}
            >
              <span style={{ flexShrink: 0, display: "flex", alignItems: "center", width: 20 }}>
                {loading === p.id ? (
                  <div style={{ width: 18, height: 18, border: "2px solid currentColor", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} />
                ) : p.icon}
              </span>
